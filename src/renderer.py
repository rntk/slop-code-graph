...
# ---------------------------------------------------------------------------
# File / module tree-map view — a second tab that collapses the call graph to
# the file level and renders it as a nested squarified treemap. Each file is a
# rectangle; the files it calls are nested inside it (so an entry point such as
# main.py becomes the outer box that contains the modules it reaches). You still
# navigate the *call* relationships — just aggregated to files instead of
# functions — which makes the module structure easy to read at a glance.
# CSS + JS injected via plain placeholders (no brace-escaping needed).
# ---------------------------------------------------------------------------
